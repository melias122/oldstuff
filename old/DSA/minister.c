...
void insert_id(treap * node, unsigned int id) {
    ...
    if(id > max){
        ...
        // ked nam pole nestaci tak si ho reallocneme
        else {
...
void lrotate(treap * node) {
	...
	// Ak existuje rodic nastavime potomka
	if(parent) {
		if(node == parent->left) {
			parent->left = child;
        }
...
// Funkcia rotuje uzol nahor az kym jeho priorita
// nie je mensia ako priorita jeho rodica
void buble_up(treap * node) {
...
void buble_down(treap * node){
    ...
    if (node) {
    	...
    	// Rotujeme nadol az kym nie sme na spodku stromu
        ...
        // Nastavime rodica na NULL a vyprazdnime uzol
        if(node->parent->left == node) {
...
// kedze mam binarny strom a nas interval moze byt
// kdekolvek musim rekurzivne prehladat cely strom
void ftreeinterval(unsigned int casOD, unsigned int casDO, treap * node){
    ...
    if (pom) {
        ...
        if ( (casOD >= pom->casOD && casOD <= pom->casDO) ||
...
void delete_by_id(unsigned short cislo_schodzky){
	...
	// Nacitam pointer z pola
	treap * pom = (treap *)pole_id[cislo_schodzky];
	buble_down(pom);
}

// Sluzi len na testovacie ucely
void testT(void){
