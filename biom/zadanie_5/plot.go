...
func main() {
	...
	prefix := strings.Split(file.Name(), "_")[0]
	...
	if err := p.Save(8*vg.Inch, 8*vg.Inch, prefix+"_fmr_fnmr.png"); err != nil {
		panic(err)
	}
	...
	// Save the plot to a PNG file.
	if err := p2.Save(8*vg.Inch, 8*vg.Inch, prefix+"_roc.png"); err != nil {
		panic(err)
	}
}
